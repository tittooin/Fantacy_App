    ...
    if (!dataFound && seriesId !== "0") {
      ...
      if (resp.ok && resp.status !== 204) {
        ...
        if (data.squads) {
          ...
          if (squadA) finalSquads.teamA = await fetchSquadPlayers(squadA, seriesId, key, apiHost, teamAId);
          if (squadB) finalSquads.teamB = await fetchSquadPlayers(squadB, seriesId, key, apiHost, teamBId);
...
async function saveToFirestore(collection, data, env) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      console.error(`\xE2\x9D\u0152 Firestore Save Error: ${e.message}`);
    }
  }
  return true;
}
